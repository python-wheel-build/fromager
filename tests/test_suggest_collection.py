"""Tests for the graph find-best-fit command and its helper functions."""
...
# ---------------------------------------------------------------------------
# CLI integration tests for find-best-fit
# ---------------------------------------------------------------------------


class TestFindBestFitCLI:
    """Integration tests for ``fromager graph find-best-fit``."""
    ...
    def test_empty_onboarding_graph(
        ...
        assert "No top-level packages" in result.stderr
        data = json.loads(result.stdout)
        assert data == []
    ...
    def test_duplicate_collection_names_fails(
        self,
        cli_runner: CliRunner,
        graph_dir: pathlib.Path,
    ) -> None:
        """Two collection files with the same stem produce a non-zero exit."""
        subdir = graph_dir / "subdir"
        subdir.mkdir()
        ds2 = _build_graph({"scipy": "1.12"}, {})
        _write_graph(ds2, subdir / "data-science.json")
        ...
        result = cli_runner.invoke(
            ...
            [
                ...
                str(graph_dir / "onboarding.json"),
                str(graph_dir / "data-science.json"),
                str(subdir / "data-science.json"),
            ],
        )
        assert result.exit_code != 0
        assert "Duplicate collection name" in result.stderr

    def test_invalid_onboarding_graph_fails(
        self,
        cli_runner: CliRunner,
        graph_dir: pathlib.Path,
    ) -> None:
        """Invalid onboarding JSON produces a non-zero exit."""
        bad_file = graph_dir / "bad-graph.json"
        ...
        assert result.exit_code != 0
        assert "Failed to load onboarding graph" in result.stderr
