...
class TestCreateRetrySession:
    ...
    def test_create_retry_session_with_github_token(self):
        """Test session creation with GitHub token from environment."""
        session = http_retry.create_retry_session()
        assert "Authorization" in session.headers
        assert session.headers["Authorization"] == "token test_token"

    @patch.dict(os.environ, {}, clear=True)
    def test_create_retry_session_without_github_token(self):
        """Test session creation without GitHub token."""
        session = http_retry.create_retry_session()
