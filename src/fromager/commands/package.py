...
def parse_distribution_option(
    distribution_type: str | DistributionType, pbi: packagesettings.PackageBuildInfo
) -> tuple[bool, bool]:
    ...
    logger.debug(
        "Using distribution type: %s (sdists: %s, wheels: %s)",
        dist_type.value,
        include_sdists,
        include_wheels,
    )
...
def _get_most_recent_package(
    upload_times: typing.Iterable[datetime.datetime | None],
) -> datetime.datetime | None:
    """Get most recent release, convert to local TZ"""
    recent = max((dt for dt in upload_times if dt is not None), default=None)
    if recent is not None:
        return recent.astimezone().replace(microsecond=0)
    else:
        return None
...
def _resolve_fromager(
    ...
    click.secho(
        f"- using distribution type '{distribution_type.value}' (sdists: {include_sdists}, wheels: {include_wheels})"
    )
    ...
    if constraint is not None:
        click.secho(
            f"- package is constrained by '--constraints-file': {constraint}.",
            bold=True,
        )
    else:
        click.secho("- package is not constrained by '--constraints-file'")
    ...
    click.secho(
        ...
        f"req: '{req}', Fromager constraint: '{constraint}'",
    )

    recent = _get_most_recent_package(candidate.upload_time for candidate in candidates)
    click.secho(
        f"- most recent release: {recent if recent else 'unknown'}",
    )
...
def _resolve_pypi(
    ...
    pbi = ctx.package_build_info(req)
    if not pbi.use_pypi_org_metadata:
        click.secho(
            "Package has pypi_org_metadata set to false, not resolving from PyPI ",
            err=True,
            bold=True,
        )
        return None
    ...
    except pypi_simple.NoSuchProjectError:
        click.secho(f"Package '{req.name}' not found on PyPI", err=True, bold=True)
        return None
    except Exception as e:
        click.secho(
    ...
    for pkg in pypi_package.packages:
        ...
        if pkg.is_yanked:
            logger.debug("%s is yanked", pkg.version)
            continue
        if req.specifier and version not in req.specifier:
            logger.debug(
                "%s is not in requirement specifier %s", version, req.specifier
            )
            continue
        if constraint is not None and version not in constraint.specifier:
            logger.debug(
                "%s is excluded by Fromager constraint %s",
                version,
                constraint,
            )
            continue
    ...
    recent = _get_most_recent_package(pypi_upload_times)
    click.secho(
        f"- most recent PyPI release: {recent if recent else 'unknown'}",
    )
...
def resolve(
    ...
    with log.req_ctxvar_context(req):
        ...
        fromager_versions = _resolve_fromager(
            wkctx, req, constraint, distribution_type, sdist_server_url
        )

        # resolve package on PyPI
        click.echo()
        pypi_versions = _resolve_pypi(wkctx, req, constraint)
        ...
        if not fromager_versions:
            click.secho()
            click.secho("Error: Fromager lookup failed", err=True, bold=True)
            sys.exit(2)
