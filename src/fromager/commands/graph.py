...
def write_dot(
    ...
    _node_color_properties = {
        "build_settings": " fillcolor=orange color=orange fontcolor=black",
        "build": " fillcolor=lightgrey color=lightgrey fontcolor=black",
        "default": " fillcolor=white color=black fontcolor=black",
        "patches": " fillcolor=green color=green fontcolor=black",
        "plugin_and_patches": " fillcolor=red color=red fontcolor=white",
        "plugin": " fillcolor=blue color=blue fontcolor=white",
        "pre_built": " fillcolor=lightblue color=lightblue fontcolor=black",
        "toplevel": " fillcolor=lightgrey color=lightgrey fontcolor=black",
    }
    ...
    for node in sorted(nodes_to_include, key=lambda x: x.key):
        ...
        if not node:
            label = "*"
        elif node.key in install_constraints and not install_only:
            label = f"{node.key} (install)"
        else:
            label = node.key

        properties = f'label="{label}" style=filled'
        ...
        properties += " ".join(_node_color_properties[t] for t in node_type)
