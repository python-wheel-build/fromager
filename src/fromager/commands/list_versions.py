...
@click.option(
    "--distribution-type",
    type=click.Choice([e.value for e in DistributionType], case_sensitive=False),
...
def list_versions(
    ...
    elif dist_type == DistributionType.WHEEL:
        include_sdists = False
        include_wheels = True
    elif dist_type == DistributionType.BOTH:
        include_sdists = True
        include_wheels = True
    else:  # DEFAULT
    ...
    versions: list[Version] = [candidate.version for candidate in candidates]

    if not versions:
        if ignore_no_versions:
            logger.warning(f"No versions found for {req}")
            return
        else:
            raise click.ClickException(f"No versions found for {req}")

    # Sort versions in increasing order and print them
    sorted_versions = sorted(versions)
    logger.info(f"Found {len(sorted_versions)} version(s)")
