...
def build_parallel(
    ...
    ready_node_keys: set[str] = set()
    exclusive_node_keys: set[str] = set()
    ...
    for node in graph.get_all_nodes():
        if node.key == dependency_graph.ROOT:
            continue
        build_dependencies = sorted(edge.destination_node.key for edge in node.children)
        logger.debug("%s -> %s", node.key, ", ".join(build_dependencies))
        topo.add(node.key, *build_dependencies)
        topo_size += 1
        if wkctx.settings.package_build_info(node.canonicalized_name).exclusive_build:
            exclusive_node_keys.add(node.key)

    # Mark as finished, detect cycles
    topo.prepare()

    logger.info("found %d packages to build", topo_size)

    built_entries: list[BuildSequenceEntry] = []
    ready_exclusive_node_keys: set[str]
    ready_nodes: DependencyNodeList
    rounds: int = 0
    ...
    with (
        ...
        while topo.is_active():
            ...
            ready_node_keys.update(topo.get_ready())

            # Check if any buildable node requires exclusive build (exclusive_build == True)
            ready_exclusive_node_keys = sorted(
                ready_node_keys.intersection(exclusive_node_keys)
            )
            if ready_exclusive_node_keys:
                # Only build the first exclusive node this round. The other
                # nodes are kept in ready_node_keys and not marked as 'done'.
                ready_nodes = [graph.nodes[ready_exclusive_node_keys[0]]]
                logger.info(
                    f"{ready_nodes[0].canonicalized_name}: requires exclusive build, running it alone this round."
                )
            else:
                ready_nodes = [graph.nodes[node_key] for node_key in ready_node_keys]

            logger.info(
                "round #%i, ready to build: %s",
                rounds,
                ", ".join(sorted(n.key for n in ready_nodes)),
            )
            ...
            for node, future in zip(ready_nodes, futures, strict=True):
                try:
                    ...
                    topo.done(node.key)
                    ready_node_keys.remove(node.key)
