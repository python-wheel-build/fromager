...
class PyprojectFixer:
    ...
    # default to setuptools's build backend
    ...
    def default_builds_system(self, doc: tomlkit.TOMLDocument) -> TomlDict:
        ...
        if BUILD_BACKEND not in build_system:
            build_system[BUILD_BACKEND] = self.default_build_backend
            logger.debug(
                "%s: adding %s = %s",
                self.name,
                BUILD_BACKEND,
            ...
            if name == "distutils":
                ...
                add.append(
                    self.overrides.replace_build_requires.get(
                        "setuptools", Requirement("setuptools")
                    )
                )
