...
def get_project_from_pypi(
    ...
    match package.status:
        case None:
            logger.debug("%s: no package status", project)
        case pypi_simple.ProjectStatus.ACTIVE:
            logger.debug("%s: project is active: %s", project, package.status_reason)
        case pypi_simple.ProjectStatus.DEPRECATED | pypi_simple.ProjectStatus.ARCHIVED:
            logger.warning(
                "%s: project is no longer active: %r: %s",
                project,
                package.status,
        ...
        case pypi_simple.ProjectStatus.QUARANTINED:
            raise ValueError(
                f"{project}: project is quarantined: {package.status_reason}"
            )
        case _:
            logger.warning(
                "%s: unknown status %r: %s",
                project,
                package.status,
                package.status_reason,
            )
    ...
    for dp in package.packages:
        ...
        if DEBUG_RESOLVER:
            logger.debug(
                "%s: candidate %r -> %r==%r", project, dp.url, dp.filename, dp.version
            )
        ...
        if (
            ...
            if DEBUG_RESOLVER:
                logger.debug(
                    f"{project}: skipping {dp.filename!r} because 'pypi_simple'"
                    "could not parse it or it's an invalid name"
                )
        ...
        if dp.package_type not in {"sdist", "wheel"}:
            if DEBUG_RESOLVER:
                logger.debug(
                    f"{project}: skipping {dp.filename!r} because it's not an "
                    f"sdist or wheel, got {dp.package_type!r}"
                )
            ignored_candidates.add(dp.filename)
            continue
        ...
        if dp.requires_python:
            try:
                ...
                if DEBUG_RESOLVER:
                    logger.debug(
                        f"{project}: skipping {dp.filename!r} because of an invalid python version specifier {dp.requires_python}: {err}"
                    )
                ignored_candidates.add(dp.filename)
                continue
            if not matched_py:
                if DEBUG_RESOLVER:
                    logger.debug(
                        f"{project}: skipping {dp.filename!r} because of python version {dp.requires_python}"
                    )
                ignored_candidates.add(dp.filename)
                continue
        ...
        try:
            ...
            if DEBUG_RESOLVER:
                logger.debug(
                    f'{project}: could not determine version for "{dp.filename!r}": {err}'
                )
            ignored_candidates.add(dp.filename)
            continue
        ...
        if tags:
            ...
            if not matching_tags and ignore_platform:
                if DEBUG_RESOLVER:
                    logger.debug(f"{project}: ignoring platform for {dp.filename!r}")
            ...
            if not matching_tags:
                if DEBUG_RESOLVER:
                    logger.debug(
                        f"{project}: ignoring {dp.filename!r} with tags {tags}"
                    )
                ignored_candidates.add(dp.filename)
                continue
        ...
        if DEBUG_RESOLVER:
            logger.debug("%s: candidate %s (%s) %s", project, dp.filename, c, dp.url)
        yield c

    if not found_candidates:
        logger.info(f"{project}: found no candidate files at {sdist_server_url}")
    elif ignored_candidates == found_candidates:
        logger.info(f"{project}: ignored all candidate files at {sdist_server_url}")
