...
class Step(enum.StrEnum):
    SDIST_STEP = "sdist"
    DIST_INFO_METADATA_STEP = "dist-info-metadata"
...
class DistInfoMetadataPatchBase(PatchBase):
    ...
    step = Step.DIST_INFO_METADATA_STEP
...
class Patches(pydantic.RootModel):
    root: list[PatchUnion]

    def _iter_patchers(
        self, step: Step, version: Version
    ) -> typing.Iterable[SdistPatchBase | DistInfoMetadataPatchBase]:
        """Get patchers, filtered by step type and when_version"""
        for patcher in self.root:
            if patcher.step != step:
                continue
            if patcher.when_version is not None and version not in patcher.when_version:
                logger.info(
                    "skipping patcher %s %s for version %s",
                    patcher.op,
                    patcher.title,
                    version,
                )
    ...
    def run_sdist_patcher(
        ...
        *,
        ...
        for patcher in self._iter_patchers(Step.SDIST_STEP, version):
            if typing.TYPE_CHECKING:
    ...
    def run_dist_info_metadata_patcher(
        ...
        *,
        ...
        for patcher in self._iter_patchers(Step.DIST_INFO_METADATA_STEP, version):
            if typing.TYPE_CHECKING:
