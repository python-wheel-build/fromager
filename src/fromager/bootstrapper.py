...
class Bootstrapper:
    def __init__(
        ...
        num_bg_threads: int = max(1, (os.cpu_count() or 2) // 2),
    ) -> None:
    ...
    def bootstrap(self, req: Requirement, req_type: RequirementType) -> None:
        ...
        while stack:
            ...
            # Phase handlers return [continuation, *new_deps] so _push_items
            # naturally puts new deps on top of the stack (processed first)
            # and submits background tasks for them.
    ...
    def _phase_prepare_source(self, item: WorkItem) -> list[WorkItem]:
        ...
        # Collect background result if a future was submitted.
        ...
        if item.pbi_pre_built:
            if prepared is not None and prepared.wheel_filename is not None:
        ...
        # Source build path
        if prepared is not None and prepared.sdist_root_dir is not None:
            # Background task already downloaded and prepared the source
