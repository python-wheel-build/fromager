...
The hook signatures are defined as `typing.Protocol` classes (see
[sandboxing_api.py](sandboxing_api.py)):

```{literalinclude} sandboxing_api.py
:start-at: class SandboxLifecycle
```
...
keyword arguments and return a `subprocess.CompletedProcess` object. Like
`subprocess.run`, the hooks also return a `CompletedProcess` object when
`returncode` is non-zero. The `run_unconfined` hook is included so users can
monitor and police unconfined calls.
...
- New methods: `BuildEnvironment.run_sandboxed`,
  `WorkContext.run_sandboxed`, `WorkContext.run_unconfined`. The function
  raise `CalledProcessError` via `CompletedProcess.check_returncode`.
...
| Landlock | Yes | Yes (6.7+) | Partial (6.12+) | No | Filesystem since 5.13, TCP since 6.7, signal/socket scoping since 6.12 |
